...
int detectCharacters(cv::Mat imgIn, cv::Rect platePos, vector<cv::Rect> *characters, vector<cv::Mat> *charImages) {
    int ret = 0;
    char nombreImagen_rec[100];
    char nombreImagen_thres[100];
    char nombreImagen_RN[100];
    cv:: Mat imageThres;

    // Crear una imagen nueva con la region de la matricula
    cv:: Mat imgROI = imgIn(platePos);
    
    sprintf(nombreImagen_rec, "./dat/imagenROI1.png");
    //imshow(nombreImagen_rec, imgROI);
    cv::imwrite(nombreImagen_rec, imgROI);
    //cv::waitKey(0);

    // Umbralizar la imagen con umbral global y adaptativo (probar los 2)
    //cv::threshold(imgROI, imageOut, 200, 250, 3);
    cv::adaptiveThreshold(imgROI, imageThres, 255,  CV_ADAPTIVE_THRESH_MEAN_C, CV_THRESH_BINARY, 9, 4);
    
    sprintf(nombreImagen_thres, "./dat/imagen_threshold1.png");
    //imshow(nombreImagen_thres, imageThres);
    cv::imwrite(nombreImagen_thres, imageThres);
    //cv::waitKey(0);


    // Llamar a la funcion removeNoise
    cv::Mat imgRN;
    removeNoise(imageThres, &imgRN);

	sprintf(nombreImagen_RN, "./dat/imagen_removeNoise1.png");
    //imshow(nombreImagen_RN, imgRN);
    cv::imwrite(nombreImagen_RN, imgRN);
    //cv::waitKey(0);

    
    // Detectar los caracteres llamando a vertHorizProjection
    vertHorizProjection(imgRN, platePos, 1, 6, characters, charImages);
 ...
 *
 * @return int alue is returned. -1 if an error occurs, 0 otherwise.
 **/
int removeNoise(cv::Mat imgIn, cv::Mat *imgOut) {
    ...
    cv::dilate(imgIn, imgFiltered, kernel);
    cv::erode(imgFiltered, imgFiltered, kernel);
    


    // eliminar el borde negro alrededor de la imagen
    cv::rectangle(imgFiltered, cv::Rect(0,0,imgIn.cols, imgIn.rows),  CV_RGB(255,255,255),8 ,8,0 );
